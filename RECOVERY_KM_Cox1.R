...
#------------------------------------------------------------
data_int <- read_csv("references/RECOVERY_Sx.csv")       # time, incidence (%)
data_clinical <- read_csv("references/RECOVERY_Ctl.csv") # time, incidence (%)
...
#------------------------------------------------------------
# 6. Restrict analysis to  4 year by censoring after 4 year
#------------------------------------------------------------
ipd_censor4 <- ipd_all %>%
  mutate(
    # If time > 4, censor at 4
    time_4 = pmin(time, 4),
    event_4 = if_else(time <= 4, event, 0)
  )

cox_4 <- coxph(Surv(time_4, event_4) ~ group, data = ipd_censor4)
summary(cox_1)

# Print HR + CI
hr_4 <- exp(coef(cox_4))
ci_4 <- exp(confint(cox_4))
cat("HR at <= 4 year =", hr_4, "\n95% CI =", ci_4[1], "to", ci_4[2], "\n")

# Plot KM up to 4 year
fit_4 <- survfit(Surv(time_4, event_4) ~ group, data = ipd_censor4)
ggsurvplot(
  fit_4, data = ipd_censor4,
  xlim = c(0, 4),
