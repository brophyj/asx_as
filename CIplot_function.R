CIplot <- function(dataset1, dataset2, group1_name, group2_name, study_name, time_unit = "years", max_y = 40) {
  ...
  if (time_unit == "months") {
    x_breaks <- seq(0, max_time, by = 12)  # yearly intervals in months
    x_label <- "Months since Randomization"
  } else {  # default to years
    x_breaks <- seq(0, max_time, by = 1)
  ...
  p_ci <- ggplot(df_ci, aes(x = time, y = ci, color = group)) +
    ...
    scale_x_continuous(limits = c(0, max_time), breaks = x_breaks) +
...
# Example of usage:
# CIplot("path/to/dataset1.csv", "path/to/dataset2.csv", "Surveillance", "Surgery", "Study XYZ", time_unit = "months", max_y = 50)
